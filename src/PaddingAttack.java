...
public class PaddingAttack {
  ...
  public String recoverPlaintext(byte[][] ciphertextBlocks) {
    ...
    ByteBuffer buffer = ByteBuffer.allocate(2);// creates a byte buffer that will hold two byte, the current and
    ...
    for (int i = 0; i < ciphertextBlocks.length; i++) {
      ...
      while (buffer.hasRemaining()) {// while loop that checks if the byte buffer still contain bytes
        byte b = buffer.get();
      ...
      byte[] recoveredBlock = recoverMessageBlock(currBlock, prevBlock);// calls the recoverMessageBlock method on the
                                                                        // current and previous block, which will give
                                                                        // us the recovered block
  ...
  private byte[] recoverMessageBlock(byte[] currentBlock, byte[] previousBlock) {// recovers the message block form the
    ...
    for (int i = 0; i <= byteBlockSize; i++) {
      for (int f = 0; f < 256; f++) {// guess checks all 256 possible bytes to find the byte that will result in the
                                     // desired output(I.E. finds the byte that will result in 1 when XOR'd)
        T[byteBlockSize - i] = (byte) f;// sets the T equal to the byte f that results in the desired output.

        // Sets the padding bytes for the T
        for (int j = 1; j < i; j++) {
          T[byteBlockSize - j] = (byte) i;
          recoveredBlock[byteBlockSize - i] = (byte) (currentBlock[byteBlockSize - i] ^ T[byteBlockSize - i] ^ i);// revocers
                                                                                                                  // the
                                                                                                                  // message
                                                                                                                  // by
                                                                                                                  // XORing
